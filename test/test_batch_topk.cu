...
bool check_large_batch_histogram_baseline_contract() {
  return radix_topk::kOptimizedSegLen == 10000 &&
         ...
         radix_topk::kLargeBatchHistogramSampleWidth == 4;
}

bool run_dispatch_boundary_gpu_case(int seg_num, int seg_len, int k, uint32_t seed) {
  ...
  if (!ok) {
    std::fprintf(stderr,
                 "dispatch boundary runtime failure seed=%u shape=(%d,%d,%d)\n",
                 seed,
                 seg_num,
                 seg_len,
                 k);
  ...
  for (int seg = 0; seg < seg_num; ++seg) {
    ...
    for (int i = 0; i < k; ++i) {
      ...
      if (actual_index != expected.indices[i] || actual_value != expected.values[i]) {
        std::fprintf(stderr,
                     "dispatch boundary seed=%u shape=(%d,%d,%d) mismatch seg=%d rank=%d: got (%d, %.8g) expected (%d, %.8g)\n",
                     seed,
                     seg_num,
                     seg_len,
                     k,
...
bool check_large_batch_dispatch_boundary() {
  return run_dispatch_boundary_gpu_case(1499, 10000, 50, 20260402u) &&
         run_dispatch_boundary_gpu_case(1500, 10000, 50, 20260403u);
}
...
int main() {
  ...
  if (!check_large_batch_dispatch_boundary()) {
    std::fprintf(stderr, "large batch dispatch boundary check failed\n");
    return 1;
  }
